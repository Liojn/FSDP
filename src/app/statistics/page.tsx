...
const categoryReductionData = [
  ...
  const calculateProgress = (reduction) => reduction;
  ...
  return (
    <div className="h-full w-full p-2 rounded-lg">
      ...
      <header className=" text-black">
        <h1 className="text-2xl font-Helvetica font-bold">Statistics Sustainability KPI</h1>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mt-2"> {/* Change to 5 columns for custom widths */}
        {metricsData.map((metric, index) => (
            ...
            >
            <h2 className="text-base font-bold text-gray-700">{metric.title}</h2>
        ...
        <div className="bg-white p-4 rounded-lg shadow-md md:col-span-3 h-[95%]"> {/* Adjusted to take 3 columns */}
            <h2 className="text-base font-bold text-gray-700">Category Reductions</h2>
            <div className="grid grid-cols-2 gap-3"> {/* Use grid for 2 columns layout */}
            {categoryReductionData.map((data, index) => (
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm text-gray-600">{data.reduction}%</span>
                </div>
                <div className="w-full h-2 bg-gray-200 rounded-lg">
                    <div
                    className={`h-full ${data.reduction >= 100 ? 'bg-red-500' : 'bg-blue-500'} rounded-lg`}
    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
      {Object.entries(categoryData).map(([category, data]) => (
        <div key={category} className="bg-white p-1 rounded-lg shadow-md relative mt-3">
