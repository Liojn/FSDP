...
const StatisticsPage = () => {
  ...
  const [categoryReductionData, setCategoryReductionData] = useState<CategoryReductionData>({});
...
// Store impact data for current and previous year
      const transformedMetricsData: Metric[] = [
        ...
        { title: `${selectedYear - 1}'s IMPACT`, value: dataPrevious.total.toFixed(2), unit: unit }
      ];
...
// Total emissions per category per year
  ...
  return (
    ...
    <div className="flex justify-start items-center text-sm font-bold space-x-6 mt-1 p-2 rounded">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mt-2"> {/* Change to 5 columns for custom widths */}
        {/* Current Year Impact */}
        <div className="">
          <MetricCard 
            title={`${year}'s IMPACT`} 
        ...
        <div className="bg-white p-2 pl-4 rounded-lg shadow-xl md:col-span-3 h-[100%]">
          <h2 className="text-base font-bold text-gray-700">Category Differences (compared to previous year)</h2>
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(categoryReductionData).map(([category, data], index) => {
          ...
          >
            ...
            <div className="flex justify-between items-center ml-1">
              <div className="flex items-baseline space-x-2">
                <p className="text-xl font-bold">{categoryData[key]?.actual || '-'}</p>
      ...
      <div className="flex flex-wrap justify-between h-screen mt-2">
        ...
        <div className="flex flex-col w-[49.5%] h-[42%] mt-5">
          <div className="bg-white h-full flex justify-center items-center rounded-lg">
            {category === "all" ? (
              <LineGraphContainer selectedYear={year} dataType={dataType} category={category} />
            ) : category === "crops" ? (
            <CropEmissionGraphContainer selectedYear={year} dataType={dataType} category={category} />
            ) : category === "livestock" ? (
            <LivestockEmissionsGraphContainer selectedYear={year} dataType={dataType} category={category} />
            ) : category === "equipment" ? (
            <EquipmentLineChart selectedYear={year} dataType={dataType} category={category} />
            ) : category === "waste" ? (
            <WasteEmissionGraphContainer selectedYear={year} dataType={dataType} category={category} />
