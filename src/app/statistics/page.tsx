  ...
  const fetchData = async (selectedYear: number, dataType: string) => {
    ...
    try {
      ...
      const categoryReductionData: CategoryReductionData = {
        all: {
          reduction:
            dataCurrent.total && dataPrevious.total
              ? calculateReduction(dataCurrent.total, dataPrevious.total)
              : "0",
        },
        equipment: {
          reduction:
            dataCurrent.calculations?.equipment &&
            dataPrevious.calculations?.equipment
              ? calculateReduction(
                  dataCurrent.calculations.equipment,
                  dataPrevious.calculations.equipment
                )
        ...
        livestock: {
          reduction:
            dataCurrent.calculations?.livestock &&
            dataPrevious.calculations?.livestock
              ? calculateReduction(
                  dataCurrent.calculations.livestock,
                  dataPrevious.calculations.livestock
                )
        ...
        crops: {
          reduction:
            dataCurrent.calculations?.crops && dataPrevious.calculations?.crops
              ? calculateReduction(
                  dataCurrent.calculations.crops,
                  dataPrevious.calculations.crops
                )
              : "0",
        },
        waste: {
          reduction:
            dataCurrent.calculations?.waste && dataPrevious.calculations?.waste
              ? calculateReduction(
                  dataCurrent.calculations.waste,
                  dataPrevious.calculations.waste
                )
              : "0",
        },
      };
      ...
      const transformedCategoryData: CategoryData = {
        ...
        equipment: {
          actual: dataCurrent.calculations.equipment.toFixed(2),
          unit: unit,
        },
        livestock: {
          actual: dataCurrent.calculations.livestock.toFixed(2),
          unit: unit,
        },
        crops: {
          actual: dataCurrent.calculations.crops.toFixed(2),
          unit: unit,
        },
        waste: {
          actual: dataCurrent.calculations.waste.toFixed(2),
          unit: unit,
        },
      };
                ...
                return (
                  <div key={index} className="flex flex-col">
                    ...
                    <div className="w-full h-2 bg-gray-200 rounded-lg">
                      <div
                        className={`h-full ${
                          isIncrease ? "bg-red-500" : "bg-lime-100"
                        } rounded-lg`}
                        style={{
