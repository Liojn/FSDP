...
const NetZeroGraph = forwardRef<HTMLDivElement, NetZeroGraphProps>(
  ({ data: propsData, isLoading: propsIsLoading = false }, ref) => {
    ...
    const [targetPercentage, setTargetPercentage] = useState<number | null>(
      null
    );
    const [minTargetPercentage, setMinTargetPercentage] = useState<
      number | null
    >(null);
          ...
          <div className="bg-white p-4 border rounded shadow">
            ...
            {netZeroTarget !== undefined && (
              <p className="text-blue-500 font-semibold">
                Net Zero Target:{" "}
                {Math.round(netZeroTarget ?? 0).toLocaleString()} kg
              </p>
            )}
          </div>
        );
              ...
              netZeroTarget !== null && (
                <Alert className="mb-4 bg-blue-50">
                  ...
                  <AlertDescription className="text-blue-700">
                    ...
                    <br />
                    Based on current reduction goals set, it would take you
                    approx <strong>{yearsToNetZero}</strong> years to reach net
                    zero emissions by <strong>{netZeroYear}</strong>.
                    <br />
                    To hit net zero emissions by <strong>2050</strong>, you have
                    to reduce your emissions by at least{" "}
                    <strong>{minTargetPercentage?.toFixed(0)}</strong> per year
                    for future years.
                  </AlertDescription>
                </Alert>
              )}
...
NetZeroGraph.displayName = "NetZeroGraph";
