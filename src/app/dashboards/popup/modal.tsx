      ...
      try {
        ...
        switch (category.toLowerCase()) {
          ...
          case 'electricity':
            ...
            break;
          case 'livestock':
            entries = data.data.livestock
              .sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
            break;
          case 'crops':
            entries = data.data.crops
              .sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
            break;
          case 'waste':
            entries = data.data.waste
              .sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
            break;
          default:
            entries = [];
          ...
          <div key={index} className="bg-gray-100 p-4 rounded shadow">
            <ul className="space-y-1">
              ...
              {category?.toLowerCase() === 'livestock' && (
                <>
                  <li><strong>Species:</strong> {detail.species}</li>
                  <li><strong>Count:</strong> {detail.count}</li>
                  <li><strong>Emissions:</strong> {formatWithUnits('emissions', detail.emissions)}</li>
                </>
              )}
              {category?.toLowerCase() === 'crops' && (
                <>
                  <li><strong>Crop Type:</strong> {detail.cropType}</li>
                  <li><strong>Area Planted:</strong> {formatWithUnits('areaPlanted', detail.areaPlanted)}</li>
                  <li><strong>Fertilizer Used:</strong> {formatWithUnits('fertilizerUsed', detail.fertilizerUsed)}</li>
                  <li><strong>Fertilizer Emissions:</strong> {formatWithUnits('fertilizerEmissions', detail.fertilizerEmissions)}</li>
                  <li><strong>Soil Emissions:</strong> {formatWithUnits('soilEmissions', detail.soilEmissions)}</li>
                  <li><strong>Total Emissions:</strong> {formatWithUnits('totalEmissions', detail.totalEmissions)}</li>
                </>
              )}
              {category?.toLowerCase() === 'waste' && (
                <>
                  <li><strong>Waste Type:</strong> {detail.wasteType}</li>
                  <li><strong>Quantity:</strong> {formatWithUnits('quantity', detail.quantity)}</li>
                  <li><strong>Emissions:</strong> {formatWithUnits('emissions', detail.emissions)}</li>
                </>
              )}
            </ul>
