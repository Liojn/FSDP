...
interface EmissionsChartProps {
  ...
  onCategoryClick: (category: string, details: string) => void; // Handler for category click
}
...
const EmissionCategoryChart: React.FC<EmissionsChartProps> = ({
  ...
  const options: ChartOptions<'doughnut'> = {
    ...
    onClick: (event, elements) => {
      if (elements.length > 0) {
        ...
        const category = data.labels ? (data.labels[index] as string) : ''; // Cast category to string
        const details = getCategoryDetails(category);
        onCategoryClick(category, details); // Use the onCategoryClick prop here
      }
    },
  };
  ...
  return (
    <div className="pt-0 min-h-[350px] pb-3">
